...
#include "client.h"
#include "handlers.h"
#include "crdt_shadow.h"   /* crdt_shadow_active: mesh copy of a redaction */
#include "hash.h"
#include "history.h"
#include "ircd.h"
 ...
 * @return CPTR_KILLED if client was squit, else 0.
 */
/* Apply a channel redaction that reached us from elsewhere (P10 REDACT or the
 * CR M 'R' mesh copy): placeholder the row, store ONE REDACT context row
 * under the origin's redact_msgid/time (one-time-per-message), fan out to
 ...
 * S2S-tag values it rides on the tree, so a receiver stores the SAME context
 * row (one-time-per-message).  Emits whenever the shadow is active; the
 * caller then keeps the tree relay off CRDT-aware links (they get this). */
int redact_mesh_mint(struct Client *from, const char *target,
                     const char *msgid, const char *redact_msgid,
                     uint64_t time_ms, const char *reason)
{
  char body[BUFSIZE], msgidbuf[64];
  if (!crdt_shadow_active() || !IsChannelName(target))
    return 0;
  ircd_snprintf(0, body, sizeof body, "%s %s %s %llu :%s",
                target, msgid, redact_msgid, (unsigned long long)time_ms,
                reason ? reason : "");
  generate_msgid(msgidbuf, sizeof msgidbuf);
  crdt_gossip_message(from, 'R', "*", msgidbuf, body);
  return 1;
}
...
int ms_redact(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (IsChannelName(target) && (chptr = FindChannel(target))) {
    ...
    /* Gateway edge (CI precedent): a REDACT that arrived over a LEGACY link
     * is minted into the mesh once here; one from a CRDT peer already rode
     * the mesh (or is the mesh copy's own legacy re-emit). */
    if (IsServer(cptr) && !IsCrdtAware(cptr))
      mesh = redact_mesh_mint(sptr, target, msgid, redact_msgid, time_ms, reason);
  ...
  /* Propagate to other servers (legacy links only when the mesh copy exists) */
  if (mesh)
    sendcmdto_set_skip_crdt_servers();
  sendcmdto_serv_butone_v3(sptr, CMD_REDACT, cptr, "%s %s :%s",
                        target, msgid, reason ? reason : "");
