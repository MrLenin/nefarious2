...
engine_loop(struct Generators *gen)
{
  ...
  while (running) {
     ...
     * entered, and a quiet node (no clients, no inbound links — it initiates
     * its overlays) has no packet to break the wait, so it boots into a coma
     * until the first external connect (live capture: epoll_wait blocked with
     * timeout=-5000 for 91 minutes).  Clamp overdue to "poll now". */
    if (wait < -1)
      wait = 0;
