...
void crdt_shadow_user_remove(struct Client *cptr, const char *comment)
{
  ...
  if (from_crdt_peer(cli_from(cptr)))  /* single-writer: peer owns this op */
    return;
  /* The quit reason rides the DELETE op's value (engine ring on the peers;
   * the tombstone keeps none) so every doc-driven exit on the mesh can say
   * why instead of the bare "Quit" -- and it arrives WITH the delta, so no
   * cross-path ordering to lose (an ephemeral CR M copy raced the delta
   * two hops out and lost, 2026-09-20). */
  crdt_user_remove_reason(&g_crdt, user_numeric(cptr, num, sizeof num), comment);
  crdt_sync_push();                    /* eager-propagate to CRDT peers */
}
...
void crdt_shadow_reconcile_user_removes(void)
{
  ...
  for (i = 0; i < nv; i++) {
    ...
    if (v && IsUser(v) && !MyUser(v) && !IsBouncerAlias(v)) {
      ...
      int has = crdt_user_quit_reason_take(&g_crdt, victims[i], why, sizeof why);
      exit_client(cli_from(v), v, v, has ? why : "Quit");   /* gateway via the now-legacy-only QUIT relay */
    }
  }
  if (nv)
    log_write(LS_SYSTEM, L_NOTICE, 0,
