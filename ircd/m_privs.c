...
int ms_privs(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  if (IsServer(sptr) || IsMeshStub(sptr) || IsMe(sptr)) {   /* IsMe: the CR M re-inject (invariant 2) */
    ...
    /* M8: the privilege set rides the user record; a PRIVS applied here
     * (services or a legacy oper's server) re-mints it -- self-skips when
     * a CRDT peer owns the user (single-writer). */
