...
static int metadata_cmd_clear(struct Client *sptr, int parc, char *parv[])
{
  ...
  if (is_channel) {
     ...
     * ORIGIN's own leaked "#chan\0*" store rows and mint the per-key doc
     * tombstones that LWW-beat the still-live SETs (metadata_account_clear
     * -> crdt_shadow_metadata_remove_key, the sanctioned raw-key
     * chokepoint — the same call channel.c's -R hook,
     * mode_channel_registered_hook(), uses for the exact same wipe).
     *
...
int ms_metadata(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
   ...
   * across the apply below so this server does NOT re-enter the doc
   * (single-writer).  Placed AFTER the limit-check early-return so a
   * dropped apply can't leak the suspend — the matching resume(0) is right
   * below, with no early return in between. */
