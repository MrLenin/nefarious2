...
static int webpush_store_receive(const char *account, const char *endpoint,
                                 const char *p256dh, const char *auth_secret,
                                 long long armed, const char *keyid,
                                 char *stored, size_t stored_len);
...
void webpush_subs_cache_invalidate(const char *account)
{
  unsigned int i;
                                 ...
                                 char *stored, size_t stored_len)
{
  ...
  if (webpush_store_get(account, endpoint, existing, sizeof(existing)) == 0
    ...
    return 0;   /* ours is at least as fresh */
  snprintf(stored, stored_len, "%s|%s|%s|%lld|%s", endpoint, p256dh,
...
int ms_webpush(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (subcmd[0] == 'R' && parc >= 6) {
    ...
    char stored[4096];
    int fresh = webpush_store_receive(account, endpoint, p256dh, auth_secret,
                                      armed, keyid, stored, sizeof(stored));
  ...
  if (subcmd[0] == 'B' && parc >= 6) {
     ...
     * sweep removed. */
    fresh = webpush_store_receive(account, endpoint, p256dh, auth_secret,
                                  armed, keyid, stored, sizeof(stored));

    /* Tier C F2-c: §17.7 gateway — a burst from a LEGACY peer at link time
     * carries subs the mesh may not have; inject them (the record just
     * written, arming time and key id included). A CRDT peer's burst is
